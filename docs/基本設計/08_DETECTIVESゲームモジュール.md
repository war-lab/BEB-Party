...
5. 犯人バリアントを抽選する（配役された全キャラクターが対象。実プレイヤーのレベルが3以上なら重み3、それ以外は重み1。[ADR-0016](../adr/0016-犯人バリアントは重みつき抽選で選ぶ.md)）
   最高レベルのプレイヤーが複数いる場合は、その中から抽選する。先頭を固定で選ぶと、初級者だけの組で毎回同じ犯人・同じ嘘になる（実測で確認）
...
* 犯人抽選のタイ: 最高レベルのプレイヤーが複数いる組で、`seed` を変えると犯人が1人に固定されないこと
