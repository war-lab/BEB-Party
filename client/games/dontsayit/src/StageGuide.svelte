...
<script lang="ts">
  ...
  const explainingText = $derived.by(() => {
    ...
    if (role === "watcher") {
      return "禁止語か正解そのものが出たら「違反」を押してください。このラウンドは答えを言わず、聞き役に回ります。";
    }
    return "画面を見る必要はありません。声を聞いて、英語でも日本語でもお題を言い当ててください。";
  });
  ...
  const text = $derived.by(() => {
    ...
    if (step === "handoff") {
      return isNextSpeaker
        ? "自分の番です。お題と禁止語を読み、用意ができたら「はじめる」を押してください。押すと時間が動き出します。"
